...
def update_grievance(user, grievance_id):
    ...
    schema = GrievanceSchema(partial=True)
    try:
        validated_data = schema.load(data)
    except ValidationError as e:
        return jsonify({"error": e.messages}), 400
    for key, value in validated_data.items():
        if hasattr(grievance, key):
...
def update_grievance_status(user, id):
    ...
    try:
        ...
        new_status = GrievanceStatus[new_status_str.upper()]
        
        # Define allowed transitions
        allowed_transitions = {
            GrievanceStatus.NEW: [GrievanceStatus.IN_PROGRESS, GrievanceStatus.ON_HOLD, GrievanceStatus.RESOLVED, GrievanceStatus.REJECTED],
            GrievanceStatus.IN_PROGRESS: [GrievanceStatus.ON_HOLD, GrievanceStatus.RESOLVED, GrievanceStatus.REJECTED],
            GrievanceStatus.ON_HOLD: [GrievanceStatus.IN_PROGRESS, GrievanceStatus.RESOLVED, GrievanceStatus.REJECTED],
            GrievanceStatus.RESOLVED: [GrievanceStatus.CLOSED],
            GrievanceStatus.REJECTED: [],
            GrievanceStatus.CLOSED: [],
        }
        
        if new_status not in allowed_transitions.get(grievance.status, []):
            return jsonify({"error": f"Invalid status transition from {grievance.status.value} to {new_status.value}"}), 400
