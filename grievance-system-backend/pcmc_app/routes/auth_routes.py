...
def guest_login():
    guest = User.query.filter_by(name="Guest User", email=None).first()
    if not guest:
...
def forgot_password():
    ...
    token = s.dumps({'user_id': user.id, 'email': email}, salt='password-reset-salt')
    frontend_url = current_app.config.get('FRONTEND_CALLBACK_URL', 'http://localhost:5500')
